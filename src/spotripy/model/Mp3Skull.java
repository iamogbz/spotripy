...
public class Mp3Skull {
    ...
    public static String getDownloadLink(String songName) throws IOException {
        ...
        String searchURL = String.format(
                Mp3Skull.url + "/search_db.php?q=%s&%s=%s",
                sanitize(songName), searchKey, curSearchToken);
        Document doc = Jsoup.connect(searchURL)
                .header("Cache-Control", "no-cache")
                .followRedirects(true)
                .userAgent(USER_AGENT)
                .data("ord", "br")
                .post(); // sort results by bitrate in descending order
        ...
        for (Element res : selection) {
            ...
            double size = matcher.find() // if the matcher found anything
                    ? Double.valueOf(matcher.group(sizePatternName)) : 0;
     ...
     * being searched for. It checks that only when the song name has things
     * like "instrumentals", "remix" etc. does it match titles that have the
     * same flags.
     *
